...
async function getData(url) {
    try {
        ...
        if (status === 200) loading.classList.add("loading-on");
...
if (localCurrencies) {
    ...
    for(let i = 0; i < data.length; i++)data[i].num = i;
    renderCurrencies(data);
}
console.log(data);
// Render function
function renderCurrencies(data) {
    ...
    data.forEach((element)=>{
        ...
        template.querySelector(".currency-value").textContent = element.Rate + " " + "so'm";
...
setTimeout(()=>{
    if (localStorage.getItem("popUp") !== "shown") showPopUp();
}, 10000);
...
sortSelect.addEventListener("change", (evt)=>{
    ...
    let sorted = [
        ...
        if (sortValue === "high-low") return Number(a.Rate) < Number(b.Rate) ? 1 : Number(a.Rate) > Number(b.Rate) ? -1 : 0;
        else if (sortValue === "low-high") return Number(a.Rate) > Number(b.Rate) ? 1 : Number(a.Rate) < Number(b.Rate) ? -1 : 0;
        else return renderCurrencies(data);
    });
...
filterInput.addEventListener("input", (evt)=>{
    ...
    let filtered = [
        ...data
    ].filter((element)=>filterValue < Number(element.Rate));
...
tableBody.addEventListener("click", (evt)=>{
    ...
    if (elStatus.classList.contains("bg-change")) // push id of elStatus
